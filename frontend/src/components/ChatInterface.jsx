...
export default function ChatInterface({ refreshDocs }) {
    ...
    const handleNewDoc = async (e) => {
      ...
      if (newFileName) {
        setAvailableDocs((prev) =>
          prev.includes(newFileName) ? prev : [...prev, newFileName],
        );
  ...
  const handleGetSummary = async () => {
    ...
    try {
      ...
      setSummaryData({
        isOpen: true,
        text: "Грешка при извличане на резюмето от сървъра.",
        isLoading: false,
      });
    }
  };
  ...
  const handleDeleteDocument = async () => {
    ...
    const confirmDelete = window.confirm(
      `Сигурни ли сте, че искате да изтриете документа "${selectedDoc}" завинаги?`,
    );
      ...
      if (fullText.includes("\n\n===SOURCES===\n")) {
        ...
        if (parts.length > 1) {
          ...
          try {
            ...
            console.error("Грешка при Sources:", e);
          }
          ...
          if (extraParts.length > 1) {
            try {
              ...
              console.error("Грешка при Follow-ups:", e);
            }
          }
        }
      }
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                if (
                                  ...
                                  alert(
                                    "Прегледът в чата е наличен само за PDF документи.",
                                  );
                                }
                              }}
